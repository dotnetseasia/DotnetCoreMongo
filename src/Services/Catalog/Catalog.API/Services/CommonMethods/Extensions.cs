...
namespace CompanyAdmin.API.Services.CommonMethods
{
    ...
    {
        private static readonly Lazy<RandomSecureVersion> _randomSecure = new Lazy<RandomSecureVersion>(() => new RandomSecureVersion());
        public static IEnumerable<T> ShuffleSecure<T>(this IEnumerable<T> source)
        {
            ...
            {
                int randomIndex = _randomSecure.Value.Next(counter, sourceArray.Length);
